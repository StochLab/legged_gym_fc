...
def load_asset(gym, sim, fix_base_link):
    asset_file = GO1
    ...
    # for all the asset’s rigid bodies/links.
    ...
    print('-----------------------------------------------------------------')
    print("Body names", num_bodies, body_names)
    print('-----------------------------------------------------------------')
    dof_names = gym.get_asset_dof_names(asset)
    num_dofs = len(dof_names)
    print("DOF names: ", num_dofs, dof_names)
    print('-----------------------------------------------------------------')
...
def create_envs(gym, sim, num_envs, envs_per_row, env_spacing):
    # load robot from urdf
    asset = load_asset(gym, sim, fix_base_link)
...
def add_terrain(gym, sim, name="slope", x_offset=2., invert=False, width=2.8):
    ...
    if name == "slope":
        ...
        heightfield[0: num_rows, :] = pyramid_stairs_terrain(new_sub_terrain(), step_width=step_width,
                                                             step_height=step_height).height_field_raw
    else:
...
def add_random_uniform_terrain(gym, sim):
    ...
    heightfield[0:1 * num_rows, :] = random_uniform_terrain(new_sub_terrain(), min_height=-0.2, max_height=0.0,
                                                            step=0.05, downsampled_scale=0.3).height_field_raw
...
def add_uneven_terrains(gym, sim):
    ...
    heightfield[0:1 * num_rows, :] = random_uniform_terrain(new_sub_terrain(), min_height=-0.1, max_height=0.1,
