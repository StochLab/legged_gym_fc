...
class Serial2RKinematics:
    ...
    def cosineRule(self, a: np.ndarray,
    ...
    def inverseKinematics(self,
                          ee_pos: np.ndarray, branch=">") -> Tuple[np.ndarray]:
        '''
...
class Serial3RKinematics:
    ...
    def inverseKinematics(self, leg_name: str,
                          ee_pos: np.ndarray,
    ...
    def forwardKinematics(self, leg_name: str,
...
class Stoch3Kinematics(Serial3RKinematics):
    ...
    '''
    def __init__(self,
                 link_parameters: list=[0.123, 0.297 , 0.347],
                 torso_dims: list=[0.533, 0.445, 0.1]):
    ...
    def isaacForwardKinematics(self,
                               q: Dict[str, th.Tensor]) -> Dict[str, th.Tensor]:
        """
    ...
    def isaacLegForwardKinematics(self, leg_name: str,
                                  q: th.Tensor) -> th.Tensor:
        """
    ...
    def getFootVelocity(self, leg_name: str, q: np.ndarray,
                        q_dot: np.ndarray) -> np.ndarray:
        """
    ...
    def getIsaacFootVelocities(self, q: Dict[str, th.Tensor],
                               q_dot: Dict[str, th.Tensor]) -> Dict[str, th.Tensor]:
        """
...
if __name__ == '__main__':
    s = Stoch3Kinematics()
    cordinates = np.array([0.0, 0.0, -0.25]).reshape(-1, 3)
    cordinates = np.concatenate([cordinates, cordinates], axis=0)
    valid, angles = s.inverseKinematics("FL", ee_pos=cordinates.reshape(-1,3), branch=">")
    cordinates = s.forwardKinematics("FL", angles)
    print("FK, \n", cordinates)
    print("IK, \n", angles)
    print("valid \n", valid)
    jac = s.Jacobian("FL", angles)
    print("Jacobian, \n", jac)

    ###############################
    # s = Serial2RKinematics()
    # cordinates = np.array([-0.5, -0.5]).reshape(-1, 2)
    # cordinates = np.concatenate([cordinates, cordinates], axis=0)
    # valid, angles = s.inverseKinematics(ee_pos=cordinates, branch=">")
    # cordinates = s.forwardKinematics(angles)
    # print("FK, \n", cordinates)
    # print("IK, \n", angles)
    # print("valid \n", valid)
    # jac = s.Jacobian(angles)
    # print("Jacobian, \n", jac)