...
class GaitData:
    """
        ...
        max_linear_xvel: torch.Tensor = 2.5/5, max_linear_yvel: torch.Tensor = 2.5/10, max_angular_vel: torch.Tensor = 2.5,
        swing_height: torch.Tensor = 0.060, torso_height: torch.Tensor = -0.25,
        stance_start: torch.Tensor = [0, PI, PI, 0], stance_duration: torch.Tensor = [PI, PI, PI, PI]
    """
    ...
    def set_new_gait_constrained(self, frequency: float, swing_height: float, torso_height: float,
            stance_start: torch.Tensor, stance_duration: torch.Tensor):
        '''
...
class TrajectoryGenerator():
    ...
    def __init__(self, device, batch_size=1, sim="isaac", gait_type='trot'):
        ...
        self.theta = torch.zeros((batch_size,1),dtype=torch.float32).to(self._device)#self.theta = 0.0
        ...
        self.front_left = LegData(device,'FL', FL, self.stoch3_kin.leg_frames[0,...].reshape((1, 3)), batch_size)
        self.front_right = LegData(device,'FR', FR, self.stoch3_kin.leg_frames[1,...].reshape((1, 3)), batch_size)
        self.back_left = LegData(device,'BL', BL, self.stoch3_kin.leg_frames[2,...].reshape((1, 3)), batch_size )
        self.back_right = LegData(device,'BR', BR, self.stoch3_kin.leg_frames[3,...].reshape((1, 3)), batch_size)
    ...
    def constrain_theta(self, theta: float) -> float:
        ...
        # if theta < 0:
        #     theta = theta + 2*PI
        theta = theta + (theta<0)*(2*PI*torch.ones(theta.shape,dtype=torch.float32).to(self._device))
        return theta
    ...
    def calculate_planar_traj(self, leg: LegData, aug_6D_twist: torch.Tensor, dt: float) -> Tuple[torch.Tensor, torch.Tensor]:
        '''
        ...
        v_leg = torch.zeros((self.Nb, 3), dtype=float).to(self._device) # Only consider linear velocity of the foot
        ...
        prev_foot_pos = torch.cat([leg.batch_prev_ee[:,0:2],
                            torch.zeros((self.Nb, 1), dtype=float).to(self._device)], dim=1).reshape((self.Nb, 3))
    ...
    def cspline_coeff(self, z0: torch.Tensor, z1: torch.Tensor, d0: torch.Tensor, d1: torch.Tensor, t: float):
        '''
    ...
    def safety_check(self, ee_pos: torch.Tensor) -> torch.Tensor:
        ...
        mag:torch.Tensor = torch.norm(safe_ee_pos, dim=1) # Magnitude of planned foot pos vector from hip (leg origin)
        if mag.any() == 0:
    ...
    def initialize_leg_state(self, action: torch.Tensor, prev_motor_angles: torch.Tensor, dt: float) -> NamedTuple:
        '''
    ...
    def generate_trajectory(self, action: torch.Tensor, prev_motor_angles: torch.Tensor, dt: float):
        '''
