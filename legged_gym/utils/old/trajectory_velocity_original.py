...
# from legged_gym.utils.logger import DataLog
import numpy as np
...
class TrajectoryGenerator():
    ...
    def update_leg_theta(self, dt: float):
        ...
        Args:
            dt : time step
        '''
        self.theta: float = self.constrain_theta(self.theta + self.gait.omega * dt) 
    ...
    def calculate_planar_traj(self, leg: LegData, aug_6D_twist: np.ndarray, dt: float) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # print("prev foot pos",type(prev_foot_pos))
        # print("leg frame",type(leg.frame))
    ...
    def generate_trajectory(self, action: th.Tensor, prev_motor_angles: th.Tensor, dt: float):
        ...
        for leg in legs:
            ...
            _, leg.batch_curr_motor_angles = self.stoch3_kin.inverseKinematics(leg.name, leg.batch_curr_ee, branch)
        ...
        self.stance_bool[..., 0] = legs.front_left.stance 
        self.stance_bool[..., 1] = legs.front_right.stance 
        self.stance_bool[..., 2] = legs.back_left.stance  
        self.stance_bool[..., 3] = legs.back_right.stance 
