...
Action space is defined as: nx18 where n is the number of batches
  action[:, 0:4  ] -> x_shift: fl fr rl rr
  action[:, 4:8  ] -> y_shift: fl fr rl rr
  action[:, 8:12 ] -> z_shift: fl fr rl rr
...
class GaitData:
    ...
    def __init__(self, frequency: float = 2.5, swing_height: float = 0.08, 
                 torso_height: float = -0.32, stance_start: np.ndarray = [0, PI, PI, 0],
                 stance_duration: np.ndarray = [PI * 1.2, PI * 1.2, PI * 1.2, PI * 1.2]):
        """Creates trot as default"""
        self.frequency = frequency
        self.max_linear_xvel = frequency / 5
        self.max_linear_yvel = frequency / 5
        self.max_angular_vel = frequency / 2
    ...
    def set_new_gait_constrained(self, frequency: float, swing_height: float, 
                                 torso_height: float, stance_start: np.ndarray, 
                                 stance_duration: np.ndarray):
        '''
    ...
    def set_new_gait_parameter(self, frequency, swing_height):
        ...
        Args:
            ...
            swing_height : height of swing foot
        '''
        frequency = np.array(frequency.cpu())
        swing_height = np.array(swing_height.cpu())
        ...
        self.max_6D_twist = np.concatenate([self.max_linear_xvel,
                                            self.max_linear_yvel,
                                            self.max_linear_yvel,  # z-vel
                                            self.max_angular_vel / 5,
                                            self.max_angular_vel / 5,
                                            self.max_angular_vel], axis=1)

        # print("max_6D_twist", self.max_6D_twist.shape)
...
class TrajectoryGenerator():
    ...
    def __init__(self, device, batch_size=1, sim="isaac", 
                 gait_type='trot', kinematics=Go1Kinematics()):
        self._device = device
        self.Nb: int = batch_size
        self.theta = np.zeros((batch_size, 1))  # self.theta = 0.0

        self.go1_kin = kinematics
        self.robot_width = self.go1_kin.torso_dims[1]
        self.robot_length = self.go1_kin.torso_dims[0]
        self.link_lengths_go1 = np.array(self.go1_kin.link_lengths)

        print('Leg frames: ', self.go1_kin.leg_frames)

        self.front_left = LegData('FL', FL, self.go1_kin.leg_frames[0, ...].reshape((1, 3)), batch_size)
        self.front_right = LegData('FR', FR, self.go1_kin.leg_frames[1, ...].reshape((1, 3)), batch_size)
        self.rear_left = LegData('RL', RL, self.go1_kin.leg_frames[2, ...].reshape((1, 3)), batch_size)
        self.rear_right = LegData('RR', RR, self.go1_kin.leg_frames[3, ...].reshape((1, 3)), batch_size)

        # warning:: changing swing height from 0.15
        if gait_type == 'trot':
            self.gait = GaitData(frequency=2.0,
                                 swing_height=0.10, torso_height=-0.30,
                                 stance_start=[PI, 0, 0, PI],
                                 stance_duration=[1.2 * PI, 1.2 * PI, 1.2 * PI, 1.2 * PI])
        ...
        self.max_6D_twist = np.array([self.gait.max_linear_xvel,
                                      self.gait.max_linear_yvel,
                                      self.gait.max_linear_yvel,  # z-vel
                                      self.gait.max_angular_vel / 5,
                                      self.gait.max_angular_vel / 5,
                                      self.gait.max_angular_vel])
        self.sim = sim

        self.stance_bool = th.tensor(
            np.concatenate([self.front_left.batch_stance_bool,
                            self.front_right.batch_stance_bool,
                            self.rear_left.batch_stance_bool,
                            self.rear_right.batch_stance_bool], axis=1), dtype=bool).to(device=self._device)

    def set_new_gait_parameter(self, frequency, stepheight):
        self.gait.set_new_gait_parameter(frequency, stepheight)
        self.max_6D_twist = self.gait.max_6D_twist
    ...
    def initialize_traj_shift(self, shifts: np.ndarray):
        ...
        self.rear_left.batch_shifts = shifts[:, RL, :].reshape((self.Nb, 3))
        self.rear_right.batch_shifts = shifts[:, RR, :].reshape((self.Nb, 3))
    ...
    def initialize_prev_motor_ang(self, prev_motor_angles: np.ndarray):
        ...
        self.rear_left.batch_prev_motor_angles = prev_motor_angles[:, 6:9].reshape((self.Nb, 3))
        self.rear_right.batch_prev_motor_angles = prev_motor_angles[:, 9:12].reshape((self.Nb, 3))
    ...
    def isStance(self, leg: LegData):
        ...
        # if (leg_phase <= self.gait.stance_duration[leg.ID]): # Having <= gives a single timestep when all the legs are in contact
        #     return True
        # else:
        #     return False
    ...
    def safety_check(self, ee_pos: np.ndarray) -> np.ndarray:
        ...
        l1, l2, l3 = self.link_lengths_go1.tolist()
        r = np.linalg.norm(safe_ee_pos[:, 1:3], axis=1)

        mask1 = np.sum(safe_ee_pos[:, 1:3] ** 2, axis=1) < self.link_lengths_go1[0] ** 2
        safe_ee_pos[mask1, 1:3] = self.link_lengths_go1[0] * safe_ee_pos[mask1, 1:3] / mag[mask1].reshape(-1, 1)
    ...
    def initialize_leg_state(self, action: np.ndarray, prev_motor_angles: np.ndarray, dt: float) -> NamedTuple:
        ...
        Args:
            ...
            dt                : control period
        Ret:
            legs : namedtuple('legs', 'front_right front_left rear_right rear_left')
        '''

        Legs = namedtuple('legs', 'front_left front_right rear_left rear_right')
        legs = Legs(front_left=self.front_left, front_right=self.front_right,
                    rear_left=self.rear_left, rear_right=self.rear_right)
    ...
    def generate_trajectory(self, action: th.Tensor, prev_motor_angles: th.Tensor, dt: float):
        '''
        ...
        aug_6D_twist: np.ndarray = action[:, 12:] * self.max_6D_twist
        ...
        for leg in legs:
            ...
            if leg.name == "RR" or leg.name == "rr" or leg.name == "rl" or leg.name == "RL":
                branch = ">"
            else:
                branch = ">"

            valid, leg.batch_curr_motor_angles = self.go1_kin.inverseKinematics(leg.name, leg.batch_curr_ee.copy(),
                                                                                   branch)
        ...
        temp_theta = np.concatenate([
            ...
            legs.rear_left.batch_curr_motor_angles,
            legs.rear_right.batch_curr_motor_angles
        ], axis=1)
        ...
        leg_foot_pos = th.cat([
            ...
            th.from_numpy(legs.rear_left.batch_curr_ee + legs.rear_left.frame).to(device=self._device),
            th.from_numpy(legs.rear_right.batch_curr_ee + legs.rear_right.frame).to(device=self._device)
        ], dim=1)
        ...
        leg_foot_vel = th.cat([
            ...
            th.from_numpy(legs.rear_left.batch_foot_vel).to(device=self._device),
            th.from_numpy(legs.rear_right.batch_foot_vel).to(device=self._device)
        ], dim=1)
        ...
        self.stance_bool[..., 2] = th.tensor(legs.rear_left.stance).reshape(-1)
        self.stance_bool[..., 3] = th.tensor(legs.rear_right.stance).reshape(-1)
