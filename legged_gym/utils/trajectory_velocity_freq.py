...
class GaitData:
    ...
    def set_new_gait_constrained(self, frequency: float, swing_height: float, torso_height: float,
            stance_start: np.ndarray, stance_duration: np.ndarray):
        '''
    ...
    def set_new_gait_parameter(self, frequency, swing_height):
        ...
        # print("frequency", self.frequency.shape)

        self.max_6D_twist = np.concatenate([np.expand_dims(self.max_linear_xvel,1),
                                np.expand_dims(self.max_linear_yvel,1), 
                                np.expand_dims(self.max_linear_yvel,1), # z-vel 
                                np.expand_dims(self.max_angular_vel/5,1), 
                                np.expand_dims(self.max_angular_vel/5,1), 
                                np.expand_dims(self.max_angular_vel,1)], axis = 1)
...
class TrajectoryGenerator():
    ...
    def __init__(self, device, batch_size=1, sim="isaac", gait_type='trot'):
        ...
        self.theta = np.zeros((batch_size,4))#self.theta = 0.0
    ...
    def reset_theta(self, idx):
        ...
        self.theta[idx,1] = 0.0
        self.theta[idx,2] = 0.0
        self.theta[idx,3] = 0.0
    ...
    def foot_step_planner(self, leg: LegData, v_leg: np.ndarray) -> np.ndarray:
        ...
        '''
        stance_time: float = (self.gait.stance_duration[leg.ID] / (2 * PI)) * (1.0 / self.gait.frequency[:,leg.ID])
        
        # print("v leg", v_leg.shape)
        # print("stance time",stance_time.shape)
        # print("leg batch shift", leg.batch_shifts.shape)
        s: np.ndarray = v_leg * stance_time.reshape(-1,1)/2 + leg.batch_shifts
    ...
    def getLegPhase(self, leg: LegData) -> float:
        leg_phase = self.theta[:,leg.ID] - self.gait.stance_start[leg.ID]
        # if (leg_phase < 0):
        #     leg_phase += 2 * PI
    ...
    def calculate_planar_traj(self, leg: LegData, aug_6D_twist: np.ndarray, dt: float) -> Tuple[np.ndarray, np.ndarray]:
        ...
        time_left = (2 * PI - leg_phase) / (2 * PI) * (1 / self.gait.frequency[:,leg.ID].reshape(-1,1))
    ...
    def calculate_vert_comp(self, leg: LegData) -> Tuple[np.ndarray, np.ndarray, bool]:
        ...
        # print("swing_phase",swing_phase.shape)
        swing_duration = 2 * PI - self.gait.stance_duration[leg.ID]
        # print("swing_duration",swing_duration.shape)
        theta_leg = PI * swing_phase / swing_duration
        # print("theta_leg",theta_leg.shape)
        dtheta_leg = PI * self.gait.omega[:,leg.ID].reshape(-1,1) / swing_duration
        # print("dtheta_leg",dtheta_leg.shape)


        # Sine function
        z: np.ndarray = self.gait.swing_height[:,leg.ID].reshape(-1,1) * np.sin(theta_leg) * \
        ...
        # print("z",z.shape)
        # print("theta leg",theta_leg.shape)
        # print("flag",flag.shape)

        
        dz: np.ndarray = self.gait.swing_height[:,leg.ID].reshape(-1,1) * np.cos(theta_leg) * \
    ...
    def safety_check(self, ee_pos: np.ndarray) -> np.ndarray:
        ...
        mag:np.ndarray = np.linalg.norm(safe_ee_pos, axis=1) # Magnitude of planned foot pos vector from hip (leg origin)
        if mag.any() == 0:
    ...
    def generate_trajectory(self, action: th.Tensor, prev_motor_angles: th.Tensor, dt: float):
        ...
        aug_6D_twist = {}
        # print("max 6D twist", self.max_6D_twist.shape)
        aug_6D_twist['FL'] = action[:, 12:] * self.max_6D_twist[:,:,0]
        aug_6D_twist['FR'] = action[:, 12:] * self.max_6D_twist[:,:,1]
        aug_6D_twist['BL'] = action[:, 12:] * self.max_6D_twist[:,:,2]
        aug_6D_twist['BR'] = action[:, 12:] * self.max_6D_twist[:,:,3]

        leg: LegData
        for leg in legs:
            # print(f"Generating trajectory for {leg.name}", f"\nphase: {leg.phase}")
            leg.batch_prev_ee = leg.batch_curr_ee # Open-loop
            xy, vxy = self.calculate_planar_traj(leg, aug_6D_twist[leg.name], dt)
            # print("xy",xy.shape)
            ...
            valid, leg.batch_curr_motor_angles = self.stoch3_kin.inverseKinematics(leg.name, leg.batch_curr_ee.copy(), branch)
        ...
        # print("legs front",legs.front_left.batch_curr_ee.shape)
        leg_foot_pos = th.cat([
        ...
        aug_6D_twist = np.concatenate([aug_6D_twist['FL'],
                                       aug_6D_twist['FR'],
                                       aug_6D_twist['BL'],
                                       aug_6D_twist['BR']], axis=1)
