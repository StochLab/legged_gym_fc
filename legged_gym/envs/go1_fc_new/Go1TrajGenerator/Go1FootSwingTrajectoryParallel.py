...
import numpy as np
from math_utils.interplation import cubicBezier, cubicBezierFirstDerivative, cubicBezierSecondDerivative
...
class FootSwingTrajectory:
    ...
    def computeSwingTrajectoryBezier(self, phase: float, swingTime: float):
        ...
        if phase < 0.5:
            ...
            za = cubicBezierSecondDerivative(self._p0[:, 2], self._p0[:, 2] + self._height, phase * 2) * 4 / (
                        swingTime * swingTime)
        else:
            ...
            za = cubicBezierSecondDerivative(self._p0[:, 2] + self._height, self._pf[:, 2], phase * 2 - 1) * 4 / (
                        swingTime * swingTime)
