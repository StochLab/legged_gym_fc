...
class OffsetDurationGait:
    ...
    pacing
    """
    def __init__(self, batchsize :int, nSegment :int, offset :np.ndarray, durations :np.ndarray, name :str):
        ...
        # self.__mpc_table = [0 for _ in range(nSegment *4)] * batchsize
        # self.__phase = [0.0] * self.batchsize
        # self.__iteration = [0] * self.batchsize
    ...
    def setIterations(self, iterationsPerMPC :int, currentIteration :int):
        ...
        self.__phase = float(currentIteration % (iterationsPerMPC * self.__nIterations)) / float \
            (iterationsPerMPC * self.__nIterations)

    # def setIterations(self, iterationsPerMPC: int, currentIteration: np.array):
    #     self.__iteration = (currentIteration / iterationsPerMPC) % self.__nIterations
    #     self.__phase = ((currentIteration % (iterationsPerMPC * self.__nIterations)) /
    #                     (iterationsPerMPC * self.__nIterations))
