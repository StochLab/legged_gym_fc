...
class StateEstimator:
    ...
    def __init__(self, quadruped:Quadruped, batch_size):
        ...
        self._foot_contact_history = np.zeros((batch_size, 4, 3))
    ...
    def update(self, root_states):
        ...
        self.result.vBody = quat_rotate_inverse(torch.tensor(self.result.orientation),
                                                torch.tensor(self.result.vWorld)).numpy()
        self.result.omegaBody = quat_rotate_inverse(torch.tensor(self.result.orientation),
                                                    torch.tensor(self.result.omegaWorld)).numpy()

        # RPY of body in the world frame
        r, p, y = get_euler_xyz(torch.tensor(self.result.orientation))
        ...
        # print("rpy: ", self.result.rpy[0])
        # print('ground R body frame: ', self.ground_R_body_frame[0])
        # print('rpyBody: ', self.result.rpyBody[0])

    def _init_contact_history(self, env_ids, foot_positions:np.ndarray):
        self._foot_contact_history[env_ids] = foot_positions[env_ids]
        for leg in range(4):
            self._foot_contact_history[env_ids, leg, 2] = -self.body_height[env_ids]

    def _update_com_position_ground_frame(self, foot_positions: np.ndarray):
        foot_contacts = self._contactPhase
        sum_foot_contacts = np.sum(foot_contacts, axis=1)
        indices = np.where(sum_foot_contacts == 0)
        if len(indices) != 0:
            self.result.position[indices, 2] = self.body_height[indices]
        indices = np.where(sum_foot_contacts != 0)
        if len(indices) != 0:
            foot_positions_ground_frame = np.einsum('bij,bjk->bik', foot_positions[indices],
                                                    np.transpose(self.ground_R_body_frame, (0, 2, 1))[indices])
            foot_heights = - foot_positions_ground_frame[:, :, 2]
            height_in_ground_frame = np.sum(foot_heights * foot_contacts[indices], axis=1) / sum_foot_contacts[indices]
            self.result.position[indices, 2] = height_in_ground_frame
